...
function sys=mdlDerivatives(t,x,uu,P)
    ...
    pedot = cos(theta)*cos(psi)*u+(sin(phi)*sin(theta)*cos(psi)+cos(theta)*sin(phi))*v+(cos(phi)*sin(theta)*cos(psi)-sin(phi)*sin(psi))*w;
    ...
    pdot = tau1*p*q-tau2*q*r+tau3*ell+tau4*n;
    qdot = tau5*p*r-tau6*(p*p-r*r)+m/P.Jy;
    rdot = tau7*p*q-tau1*q*r+tau4*ell+tau8*n;
